...
# ID3 Implementation
def ID3(data, attributes, target_attribute, depth, IG_variants, missingVal=True):
    ...
    for value in data[best_attribute].unique():
        ...
        if not subset.empty:
            subtree = ID3(subset, remaining_attributes, target_attribute, depth - 1, IG_variants, missingVal)
            tree[best_attribute][value] = subtree

    if missingVal:
        tree[best_attribute]["unknown"] = mode(target_values)[0][0]

    return tree

# Helper function to predict a single example
def predict_example(example, decision_tree, missingVal=True):
    ...
    if isinstance(decision_tree, dict):
        ...
        if attr != "unknown":
            ...
            if value in decision_tree[attr]:
                ...
                return predict_example(example, subtree, missingVal)
            elif missingVal:
                return decision_tree[attr]["unknown"]
            else:
                return mode(train_data['y'].values)[0][0]
        else:
            return decision_tree[attr]["unknown"]

# Helper function to predict a list of examples
def predict_examples(examples, decision_tree, missingVal=True):
    return [predict_example(example, decision_tree, missingVal) for example in examples]
...
for depth in max_depths:
    for variants in IG_variants:
        ...
        tree = ID3(train_data, attributes_copy, 'y', depth, variants, missingVal=True)

        train_predictions = predict_examples(train_data.to_dict(orient='records'), tree, missingVal=True)
        test_predictions = predict_examples(test_data.to_dict(orient='records'), tree, missingVal=True)
